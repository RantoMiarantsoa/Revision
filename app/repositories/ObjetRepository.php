<?php
    class ObjetRepository {
        ...
        public function getAll() {
            $stmt = $this->pdo->query("SELECT * FROM takalo_Objet");
            return $stmt->fetchAll(\PDO::FETCH_ASSOC);
        }

        public function getById($id) {
            $stmt = $this->pdo->prepare("SELECT * FROM takalo_Objet WHERE id_obj = ?");
            $stmt->execute([$id]);
        ...
        public function update($id, $nom, $description, $prix, $id_cat, $id_user) {
            $stmt = $this->pdo->prepare("UPDATE takalo_Objet SET nom = ?, description = ?, prix = ?, id_cat = ?, id_user = ? WHERE id_obj = ?");
            return $stmt->execute([$nom, $description, $prix, $id_cat, $id_user, $id]);
        }

        public function delete($id) {
            $stmt = $this->pdo->prepare("DELETE FROM takalo_Objet WHERE id_obj = ?");
